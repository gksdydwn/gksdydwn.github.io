...
category: "Project"
subcategory: "블로그 구축"
tags: ["Astro", "GitHubPages", "Obsidian"]
completionRate: 80
---
...
처음에는 모든 것을 `tags` 한 칸에 몰아넣으려 했는데, 막상 정리해 보니 **성격이 다른 두 가지가 섞여 있었습니다.**

* "이 글이 **어떤 종류의** 글인가" → 서랍에 넣는 일 (Study / Project …)
* "이 글이 **무엇에 관한** 글인가" → 꼬리표를 다는 일 (Astro / Obsidian …)

앞의 것은 글 하나에 하나뿐이고, 뒤의 것은 여러 개가 붙습니다. 이 둘을 한 칸에 두면 좌측 트리에서 같은 글이 여러 서랍에 동시에 나타나 버립니다. 그래서 칸을 셋으로 나눴습니다.

### 대분류 `category` — 글 하나당 하나, 필수

| 값 | 용도 |
| --- | --- |
| `Study` | 코딩 공부(언어, 라이브러리)와 CS 내용 정리 |
| `Project` | 나만의 소소한 토이 프로젝트 개발기 누적 |
| `Interest` | 개인적인 관심사와 생각 정리 |

목록은 `src/content.config.ts`에 `z.enum`으로 고정해 두었습니다. 덕분에 `Projct`처럼 오타를 내면 **배포되기 전에 빌드가 파일 이름과 함께 잡아줍니다.** 분류를 늘리고 싶으면 그 배열에 단어를 추가하면 됩니다.

### 소분류 `subcategory` — 선택

`Study` 아래의 `Python`, `CS`처럼 대분류를 한 번 더 쪼갤 때 씁니다. 대분류와 달리 계속 늘어나는 성격이라 목록을 고정하지 않았습니다. 안 적으면 대분류 바로 아래에 놓입니다.

### 태그 `tags` — 검색용 꼬리표, 여러 개

분류와 달리 글 하나가 여러 개를 가질 수 있습니다. 헤더의 검색(`Ctrl + K`)이 제목·설명·분류와 함께 이 태그들을 훑습니다.
...
---
title: "파이썬 리스트 정리"
category: "Study"        # 서랍 (고정된 값 중 하나)
subcategory: "Python"    # 서랍 속 칸막이 (자유, 생략 가능)
tags: ["Python", "자료형"] # 꼬리표 (자유, 여러 개)
---
```

여기서 배운 것 하나. **프론트매터에 아무리 적어도 스키마에 등록하지 않으면 조용히 사라집니다.** 처음에 `tags`를 스키마에 넣지 않은 채로 글에만 적어 두었더니, 빌드는 멀쩡히 성공하는데 좌측 트리만 텅 비어 있었습니다. 에러가 났다면 금방 알았을 텐데 아무 말이 없어서 한참 헤맸습니다.
